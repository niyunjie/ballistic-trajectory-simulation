...
std::string formatBoundaryPoints(const std::vector<BoundaryPoint>& boundary) {
  ...
  for (std::size_t i = 0; i < boundary.size(); ++i) {
    ...
    out << "{"
        ...
        << "\"intercepted\":" << (boundary[i].intercepted ? "true" : "false")
        << ","
        << "\"interceptorSiteIndex\":" << boundary[i].interceptorSiteIndex << ","
        << "\"interceptTimeS\":" << formatDouble(boundary[i].interceptTime, 3)
        << ","
        << "\"interceptLonDeg\":"
        << formatDouble(boundary[i].interceptLongitudeDeg, 6) << ","
        << "\"interceptLatDeg\":"
        << formatDouble(boundary[i].interceptLatitudeDeg, 6) << ","
        << "\"interceptHeightM\":"
        << formatDouble(boundary[i].interceptHeightM, 2) << ","
        << "\"interceptMarginM\":"
        << formatDouble(boundary[i].interceptMarginM, 2) << "}";
  }
  out << "]";
  return out.str();
}

std::string formatInterceptAssessment(const InterceptAssessment& assessment) {
  std::ostringstream out;
  out << "{"
      << "\"enabled\":" << (assessment.enabled ? "true" : "false") << ","
      << "\"intercepted\":" << (assessment.intercepted ? "true" : "false") << ","
      << "\"siteIndex\":" << assessment.siteIndex << ","
      << "\"siteName\":\"" << assessment.siteName << "\","
      << "\"interceptTimeS\":" << formatDouble(assessment.interceptTime, 3)
      << ","
      << "\"interceptLonDeg\":"
      << formatDouble(assessment.interceptLongitudeDeg, 6) << ","
      << "\"interceptLatDeg\":"
      << formatDouble(assessment.interceptLatitudeDeg, 6) << ","
      << "\"interceptHeightM\":"
      << formatDouble(assessment.interceptHeightM, 2) << ","
      << "\"slantRangeM\":" << formatDouble(assessment.slantRangeM, 2) << ","
      << "\"availableRangeM\":"
      << formatDouble(assessment.availableRangeM, 2) << ","
      << "\"marginM\":" << formatDouble(assessment.marginM, 2) << "}";
  return out.str();
}
...
std::string formatTargetSolution(const TargetSolution& solution) {
  ...
  out << "{"
      ...
      << "\"effectiveReachable\":"
      << (solution.effectiveReachable ? "true" : "false") << ","
      ...
      << "\"interceptAssessment\":"
      << formatInterceptAssessment(solution.interceptAssessment) << ","
...
void LiveWriter::writeRunningState(
  ...
  state.effectiveAttackBoundary = effectiveBoundary;
  ...
  out << "{"
      ...
      << "\"effectiveAttackBoundary\":"
      << formatBoundaryPoints(effectiveBoundary) << ","
      << "\"attackTrajectories\":"
...
void LiveWriter::writeFinalState(const FinalResult& finalResult) {
  ...
  state.effectiveAttackBoundary = finalResult.effectiveAttackBoundary;
  ...
  out << "{"
      ...
      << "\"effectiveAttackBoundary\":"
      << formatBoundaryPoints(finalResult.effectiveAttackBoundary) << ","
      << "\"interceptedAttackBoundary\":"
      << formatBoundaryPoints(finalResult.interceptedAttackBoundary) << ","
      << "\"attackTrajectories\":"
