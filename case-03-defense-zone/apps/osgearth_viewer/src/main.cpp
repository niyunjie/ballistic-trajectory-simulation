...
static std::string BuildAttackBoundaryCsv(
  ...
  out << "heading_deg,theta_deg,range_m,longitude_deg,latitude_deg,top_height_m,"
         "intercepted,interceptor_site_index,intercept_time_s,intercept_longitude_deg,"
         "intercept_latitude_deg,intercept_height_m,intercept_margin_m\n";
  for (const auto& point : boundary) {
    out << FormatNumber(point.headingDeg, 4) << "," << FormatNumber(point.thetaDeg, 4)
        ...
        << (point.intercepted ? "1" : "0") << ","
        << point.interceptorSiteIndex << ","
        << FormatNumber(point.interceptTime, 3) << ","
        << FormatNumber(point.interceptLongitudeDeg, 6) << ","
        << FormatNumber(point.interceptLatitudeDeg, 6) << ","
        << FormatNumber(point.interceptHeightM, 2) << ","
        << FormatNumber(point.interceptMarginM, 2) << "\n";
  }
  return out.str();
}
...
static std::string BuildAnalysisSummaryCsv(const ballistics::SolverConfig& config,
  ...
  std::size_t interceptedHeadingCount = 0;
  for (const auto& point : result.attackBoundary) {
    if (point.intercepted) {
      ++interceptedHeadingCount;
    }
  }
  std::size_t effectiveHeadingCount = 0;
  for (const auto& point : result.effectiveAttackBoundary) {
    if (point.range > 1.0) {
      ++effectiveHeadingCount;
    }
  }
  ...
  out << "alpha_schedule,peak_alpha_deg,launch_longitude_deg,launch_latitude_deg,launch_height_m,initial_speed_mps,"
         ...
         "effective_attack_heading_count,intercepted_attack_heading_count,method_count,"
         "target_enabled,target_reachable,target_effective_reachable,target_miss_distance_m\n";
  out << EscapeCsvField(AlphaScheduleName(config.vehicle.alphaScheduleMode)) << ","
      ...
      << (result.targetSolution.effectiveReachable ? "1" : "0") << ","
...
static std::string BuildRunParametersCsv(const ballistics::SolverConfig& config) {
  ...
  out << "defense_enabled," << (config.defense.enabled ? "1" : "0") << "\n";
  out << "defense_evaluation_samples," << config.defense.evaluationSamples << "\n";
  out << "defense_intercept_tolerance_m,"
      << FormatNumber(config.defense.interceptToleranceM, 3) << "\n";
  out << "defense_site_count," << config.defense.sites.size() << "\n";
  for (std::size_t i = 0; i < config.defense.sites.size(); ++i) {
    const auto& site = config.defense.sites[i];
    const std::string prefix = "defense_site_" + std::to_string(i + 1) + "_";
    out << prefix << "enabled," << (site.enabled ? "1" : "0") << "\n";
    out << prefix << "name," << EscapeCsvField(site.name) << "\n";
    out << prefix << "longitude_deg," << FormatNumber(site.longitudeDeg, 6) << "\n";
    out << prefix << "latitude_deg," << FormatNumber(site.latitudeDeg, 6) << "\n";
    out << prefix << "height_m," << FormatNumber(site.heightM, 3) << "\n";
    out << prefix << "reaction_delay_s," << FormatNumber(site.reactionDelayS, 3) << "\n";
    out << prefix << "interceptor_speed_mps,"
        << FormatNumber(site.interceptorSpeedMps, 3) << "\n";
    out << prefix << "max_range_m," << FormatNumber(site.maxRangeM, 3) << "\n";
    out << prefix << "min_intercept_height_m,"
        << FormatNumber(site.minInterceptHeightM, 3) << "\n";
    out << prefix << "max_intercept_height_m,"
        << FormatNumber(site.maxInterceptHeightM, 3) << "\n";
  }
  return out.str();
}
...
static std::string BuildComputedDataMarkdown(const ballistics::SolverConfig& config,
  ...
  if (!result.attackBoundary.empty()) {
    ...
    std::size_t interceptedHeadings = 0;
    std::size_t effectiveHeadings = 0;
    for (const auto& point : result.attackBoundary) {
      minRange = std::min(minRange, point.range);
      maxRange = std::max(maxRange, point.range);
      if (point.intercepted) {
    ...
    out << "- Boundary spread: " << FormatNumber((maxRange - minRange) / 1000.0, 3)
        << " km\n";
    out << "- Effective attack headings after defense filtering: " << effectiveHeadings
        << "\n";
    out << "- Intercepted attack headings: " << interceptedHeadings << "\n\n";
  }
  out << "## Target Data\n";
  if (target.enabled) {
    out << "- Reachable: " << (target.reachable ? "Yes" : "No") << "\n";
    out << "- Effective reachable after defense: "
        << (target.effectiveReachable ? "Yes" : "No") << "\n";
    ...
    if (target.interceptAssessment.intercepted) {
      out << "- Intercepted by: " << target.interceptAssessment.siteName << "\n";
      out << "- Intercept time: "
          << FormatNumber(target.interceptAssessment.interceptTime, 3) << " s\n";
      out << "- Intercept point: "
          << FormatNumber(target.interceptAssessment.interceptLongitudeDeg, 4) << " deg, "
          << FormatNumber(target.interceptAssessment.interceptLatitudeDeg, 4) << " deg, "
          << FormatNumber(target.interceptAssessment.interceptHeightM / 1000.0, 3)
          << " km\n";
    }
  } else {
...
class BallisticApp final : public oe::ImGuiEventHandler {
 ...
 private:
  ...
  void addAttackZoneMarkers(const std::vector<ballistics::BoundaryPoint>& boundary,
                            const osg::Vec4f& color, float radiusM,
                            double heightM = 2'400.0) {
    for (const auto& point : boundary) {
      dynamic_->addChild(
          sphere(point.longitudeDeg, point.latitudeDeg, heightM, radiusM, color));
    }
  }

  const ballistics::BoundaryPoint* findBoundaryPointForHeading(
      const std::vector<ballistics::BoundaryPoint>& boundary, double headingDeg) const {
    for (const auto& point : boundary) {
      if (std::abs(point.headingDeg - headingDeg) < 1e-3) {
        return &point;
      }
    }
    return nullptr;
  ...
  bool exportBundle(const ballistics::LiveState& state, std::filesystem::path& exportDir,
    ...
    try {
      ...
      const bool ok =
          ...
          WriteTextFile(exportDir / "effective-attack-boundary.csv",
                        BuildAttackBoundaryCsv(result.effectiveAttackBoundary)) &&
          WriteTextFile(exportDir / "intercepted-attack-boundary.csv",
                        BuildAttackBoundaryCsv(result.interceptedAttackBoundary)) &&
  ...
  void drawNumericalNotes(const ballistics::LiveState& state, bool done) const {
    ...
    if (done && state.finalResult.targetSolution.enabled &&
        state.finalResult.targetSolution.reachable &&
        !state.finalResult.targetSolution.effectiveReachable) {
      if (!hasNotes) {
        ImGui::SeparatorText("Numerical Notes");
        hasNotes = true;
      }
      ImGui::TextColored(
          ...
          "The target is inside the attack envelope, but the defense layer found an "
          "intercept window before impact.");
    }
  ...
  void rebuildScene(const ballistics::LiveState& state, double animationProgress = 1.0) {
    ...
    if (showDefenseSites_ && config_.defense.enabled) {
      for (const auto& site : config_.defense.sites) {
        if (!site.enabled) {
          continue;
        }
        dynamic_->addChild(sphere(site.longitudeDeg, site.latitudeDeg, site.heightM, 13'000.0f,
                                   osg::Vec4f(0.98f, 0.76f, 0.18f, 0.92f)));
        dynamic_->addChild(line({world(site.longitudeDeg, site.latitudeDeg, 0.0),
                                 world(site.longitudeDeg, site.latitudeDeg,
                                       std::max(site.heightM, 8'000.0))},
                                osg::Vec4f(1.0f, 0.82f, 0.28f, 0.28f), 1.8f));
        char siteText[196];
        std::snprintf(siteText, sizeof(siteText), "%s\nDelay %.0f s | %.0f km",
                      site.name.c_str(), site.reactionDelayS, site.maxRangeM / 1000.0);
        dynamic_->addChild(label(site.longitudeDeg, site.latitudeDeg,
                                 std::max(site.heightM, 10'000.0), siteText, 14, -14));
      }
    }

    if (showRawAttackZone_ && state.attackBoundary.size() >= 3) {
    ...
    if (showEffectiveAttackZone_ && state.effectiveAttackBoundary.size() >= 3) {
      const auto effectiveOuter =
          boundaryLoop(state.effectiveAttackBoundary, 0.042, 1'100.0);
      const auto effectiveFootprint =
          boundaryLoop(state.effectiveAttackBoundary, 0.014, 620.0);
      dynamic_->addChild(
          attackZone(effectiveFootprint, osg::Vec4f(0.18f, 0.86f, 0.46f, 0.12f)));
      dynamic_->addChild(line(path(effectiveFootprint),
                              osg::Vec4f(0.32f, 0.95f, 0.54f, 0.46f), 3.6f));
      dynamic_->addChild(line(path(effectiveOuter),
                              osg::Vec4f(0.68f, 1.0f, 0.62f, 0.96f), 9.6f));
      addAttackZoneMarkers(state.effectiveAttackBoundary,
                           osg::Vec4f(0.62f, 1.0f, 0.56f, 0.92f), 8'400.0f, 2'800.0);
    }

    if (showInterceptMarkers_ && showRawAttackZone_ && showEffectiveAttackZone_ &&
        state.attackBoundary.size() >= 3 &&
        state.effectiveAttackBoundary.size() == state.attackBoundary.size()) {
      for (std::size_t i = 0; i < state.attackBoundary.size(); ++i) {
        const auto& rawA = state.attackBoundary[i];
        const auto& rawB = state.attackBoundary[(i + 1) % state.attackBoundary.size()];
        const auto& effA = state.effectiveAttackBoundary[i];
        const auto& effB =
            state.effectiveAttackBoundary[(i + 1) % state.effectiveAttackBoundary.size()];
        const double removedA = std::max(0.0, rawA.range - effA.range);
        const double removedB = std::max(0.0, rawB.range - effB.range);
        if (removedA < 1'000.0 && removedB < 1'000.0) {
          continue;
        }

        const double rawHeightA = std::max(1'500.0, rawA.topHeight * 0.032);
        const double rawHeightB = std::max(1'500.0, rawB.topHeight * 0.032);
        const double effHeightA = std::max(800.0, std::max(0.0, effA.topHeight) * 0.022);
        const double effHeightB = std::max(800.0, std::max(0.0, effB.topHeight) * 0.022);
        std::vector<ballistics::PlotPoint> removedSector = {
            {0.0, rawA.longitudeDeg, rawA.latitudeDeg, rawHeightA, 0.0, 0.0, 0.0},
            {0.0, rawB.longitudeDeg, rawB.latitudeDeg, rawHeightB, 0.0, 0.0, 0.0},
            {0.0, effB.longitudeDeg, effB.latitudeDeg, effHeightB, 0.0, 0.0, 0.0},
            {0.0, effA.longitudeDeg, effA.latitudeDeg, effHeightA, 0.0, 0.0, 0.0},
            {0.0, rawA.longitudeDeg, rawA.latitudeDeg, rawHeightA, 0.0, 0.0, 0.0},
        };
        dynamic_->addChild(
            attackZone(removedSector, osg::Vec4f(1.0f, 0.24f, 0.16f, 0.12f)));
        dynamic_->addChild(line(path(removedSector),
                                osg::Vec4f(1.0f, 0.34f, 0.24f, 0.18f), 1.8f));
      }
    }

    if (showAttackSampleTrajectories_) {
      for (std::size_t i = 0; i < state.attackTrajectories.size(); ++i) {
        const double headingDeg = i < state.attackTrajectoryHeadings.size()
                                      ? state.attackTrajectoryHeadings[i]
                                      : 0.0;
        const auto* boundaryPoint = findBoundaryPointForHeading(state.attackBoundary, headingDeg);
        const bool intercepted = boundaryPoint != nullptr && boundaryPoint->intercepted;
        const osg::Vec4f c = intercepted ? osg::Vec4f(1.0f, 0.42f, 0.30f, 0.26f)
                                         : ((i % 2 == 0)
                                                ? osg::Vec4f(0.12f, 0.98f, 1.0f, 0.32f)
                                                : osg::Vec4f(0.48f, 1.0f, 0.30f, 0.28f));
        dynamic_->addChild(line(path(state.attackTrajectories[i]), c, intercepted ? 4.8f : 5.85f));
      }
    }

    if (showInterceptMarkers_) {
      std::size_t labeledIntercepts = 0;
      for (const auto& point : state.attackBoundary) {
        if (!point.intercepted) {
          continue;
        }
        dynamic_->addChild(sphere(point.interceptLongitudeDeg, point.interceptLatitudeDeg,
                                   std::max(point.interceptHeightM, 8'000.0), 8'800.0f,
                                   osg::Vec4f(1.0f, 0.36f, 0.28f, 0.86f)));
        dynamic_->addChild(line({world(point.longitudeDeg, point.latitudeDeg, 0.0),
                                 world(point.interceptLongitudeDeg, point.interceptLatitudeDeg,
                                       std::max(point.interceptHeightM, 8'000.0))},
                                osg::Vec4f(1.0f, 0.42f, 0.30f, 0.20f), 1.6f));
        if (labeledIntercepts < 4) {
          char interceptText[160];
          std::snprintf(interceptText, sizeof(interceptText),
                        "Intercept %.0f deg\n%.0f s | %.0f km",
                        point.headingDeg, point.interceptTime,
                        point.interceptHeightM / 1000.0);
          dynamic_->addChild(label(point.interceptLongitudeDeg, point.interceptLatitudeDeg,
                                   std::max(point.interceptHeightM, 12'000.0),
                                   interceptText, 12, -12));
          ++labeledIntercepts;
        }
      }
    ...
    if (done) {
      ...
      if (target.enabled) {
        const osg::Vec4f tc =
            !target.reachable ? osg::Vec4f(1.0f, 0.54f, 0.30f, 0.95f)
                              : (target.effectiveReachable
                                     ? osg::Vec4f(1.0f, 0.37f, 0.66f, 0.95f)
                                     : osg::Vec4f(1.0f, 0.76f, 0.18f, 0.95f));
        ...
        if (target.reachable && target.trajectoryHermite.size() >= 2) {
          ...
          if (showInterceptMarkers_ && target.interceptAssessment.intercepted) {
            dynamic_->addChild(sphere(target.interceptAssessment.interceptLongitudeDeg,
                                       target.interceptAssessment.interceptLatitudeDeg,
                                       std::max(target.interceptAssessment.interceptHeightM,
                                                8'000.0),
                                       11'500.0f,
                                       osg::Vec4f(1.0f, 0.22f, 0.16f, 0.96f)));
            char targetInterceptText[192];
            std::snprintf(targetInterceptText, sizeof(targetInterceptText),
                          "Target blocked\n%s\n%.0f s | %.0f km",
                          target.interceptAssessment.siteName.c_str(),
                          target.interceptAssessment.interceptTime,
                          target.interceptAssessment.interceptHeightM / 1000.0);
            dynamic_->addChild(label(target.interceptAssessment.interceptLongitudeDeg,
                                     target.interceptAssessment.interceptLatitudeDeg,
                                     std::max(target.interceptAssessment.interceptHeightM,
                                              12'000.0),
                                     targetInterceptText, 14, -14));
          }
        } else {
  ...
  void drawPanel(const ballistics::LiveState& state) {
    ...
    ImGui::SeparatorText("Scene Layers");
    ImGui::Checkbox("Show raw attack zone", &showRawAttackZone_);
    ImGui::Checkbox("Show effective attack zone", &showEffectiveAttackZone_);
    ImGui::Checkbox("Show intercepted points", &showInterceptMarkers_);
    ImGui::Checkbox("Show defense sites", &showDefenseSites_);
    ImGui::Checkbox("Show sample attack trajectories", &showAttackSampleTrajectories_);
    ...
    if (done) {
      ...
      if (ImGui::BeginTable("computed-data-table", 2,
                            ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersOuter |
                                ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingStretchProp)) {
        std::size_t effectiveHeadingCount = 0;
        for (const auto& point : finalResult.effectiveAttackBoundary) {
          if (point.range > 1.0) {
            ++effectiveHeadingCount;
          }
        }
        ...
        KeyValueRow("Effective attack headings",
                    FormatText("%llu",
                               static_cast<unsigned long long>(effectiveHeadingCount)));
        KeyValueRow("Defense sites",
                    FormatText("%llu",
                               static_cast<unsigned long long>(
                                   solvedConfig.defense.sites.size())));
    ...
    if (done && state.finalResult.targetSolution.enabled) {
      ...
      if (ImGui::BeginTable("target-table", 2,
        ...
        KeyValueRow("After defense", t.effectiveReachable ? "Still reachable"
                                                           : "Blocked by defense",
                    t.effectiveReachable ? statusColor
                                         : ImVec4(1.0f, 0.48f, 0.36f, 1.0f));
        ...
        if (t.interceptAssessment.intercepted) {
          KeyValueRow("Intercept site", t.interceptAssessment.siteName);
          KeyValueRow("Intercept time",
                      FormatText("%.2f s", t.interceptAssessment.interceptTime));
          KeyValueRow("Intercept point",
                      FormatText("%.2f deg, %.2f deg, %.1f km",
                                 t.interceptAssessment.interceptLongitudeDeg,
                                 t.interceptAssessment.interceptLatitudeDeg,
                                 t.interceptAssessment.interceptHeightM / 1000.0));
        }
        if (!t.reachable) {
  ...
  bool animate_ = false;
  bool focus_ = true;
  bool showRawAttackZone_ = true;
  bool showEffectiveAttackZone_ = true;
  bool showInterceptMarkers_ = true;
  bool showDefenseSites_ = true;
  bool showAttackSampleTrajectories_ = true;
