...
static double WrapHeadingDeg(double degrees) {
  while (degrees < 0.0) {
    degrees += 360.0;
  }
  while (degrees >= 360.0) {
...
static std::string CardinalDirectionLabel(double psiDeg) {
  static const std::array<const char*, 8> kLabels = {"North", "North-East", "East",
                                                     "South-East", "South", "South-West",
                                                     "West", "North-West"};
  const double wrapped = WrapHeadingDeg(psiDeg);
  const int index = static_cast<int>(std::floor((wrapped + 22.5) / 45.0)) % 8;
  return kLabels[static_cast<std::size_t>(index)];
}

static std::string InitialDirectionText(double thetaDeg, double psiDeg) {
  const double wrappedPsi = WrapHeadingDeg(psiDeg);
  const double absTheta = std::abs(thetaDeg);
  const char* verticalSense =
      thetaDeg >= 0.0 ? "above the local horizontal" : "below the local horizontal";
  return FormatText(
      "Current launch direction: %s on the ground (psi_v0 = %.1f deg, clockwise from north), %.1f deg %s.",
      CardinalDirectionLabel(wrappedPsi).c_str(), wrappedPsi, absTheta,
      verticalSense);
}
...
static std::string BuildAnalysisSummaryCsv(const ballistics::SolverConfig& config,
  ...
  std::ostringstream out;
  out << "launch_longitude_deg,launch_latitude_deg,launch_height_m,initial_speed_mps,"
  ...
  out << FormatNumber(config.vehicle.initialLongitudeDeg, 6) << ","
...
static std::string BuildRunParametersCsv(const ballistics::SolverConfig& config) {
  ...
  out << "ga_mutation_sigma_deg," << FormatNumber(config.search.geneticMutationSigmaDeg, 4)
      << "\n";
  out << "alpha_min_deg," << FormatNumber(config.search.alphaMinDeg, 4) << "\n";
  out << "alpha_max_deg," << FormatNumber(config.search.alphaMaxDeg, 4) << "\n";
  out << "beta_min_deg," << FormatNumber(config.search.betaMinDeg, 4) << "\n";
  out << "beta_max_deg," << FormatNumber(config.search.betaMaxDeg, 4) << "\n";
  out << "h_c_min_m," << FormatNumber(config.search.controlHeightMinM, 3) << "\n";
  out << "h_c_max_m," << FormatNumber(config.search.controlHeightMaxM, 3) << "\n";
...
static std::string BuildComputedDataMarkdown(const ballistics::SolverConfig& config,
  ...
  out << "- Alpha bounds: [" << FormatNumber(config.search.alphaMinDeg, 2) << ", "
      << FormatNumber(config.search.alphaMaxDeg, 2) << "] deg\n";
  out << "- Beta bounds: [" << FormatNumber(config.search.betaMinDeg, 2) << ", "
      << FormatNumber(config.search.betaMaxDeg, 2) << "] deg\n";
  out << "- h_c bounds: [" << FormatNumber(config.search.controlHeightMinM / 1000.0, 2)
      << ", " << FormatNumber(config.search.controlHeightMaxM / 1000.0, 2) << "] km\n";
  out << "- Enabled methods: " << EnabledMethodsText(config) << "\n\n";
  out << "## Optimal Trajectory\n";
  out << "- Terminal theta: " << FormatNumber(result.optimalThetaDeg, 4) << " deg\n";
  out << "- Terminal heading: " << FormatNumber(result.optimalPsiDeg, 4) << " deg\n";
  ...
  if (target.enabled) {
    out << "- Attack zone: not generated in target-point mode\n\n";
  } else {
    out << "- Valid attack-zone headings: " << result.attackBoundary.size() << "\n\n";
  }
  if (!target.enabled && !result.attackBoundary.empty()) {
  ...
  if (target.enabled) {
    ...
    out << "- Closest-approach miss distance: "
        << FormatNumber(target.missDistance / 1000.0, 3) << " km\n";
    if (std::isfinite(target.impactDistance)) {
      out << "- Impact miss distance: "
          << FormatNumber(target.impactDistance / 1000.0, 3) << " km\n";
    }
    out << "- Closest theta: " << FormatNumber(target.thetaDeg, 4) << " deg\n";
    out << "- Closest psi: " << FormatNumber(target.psiDeg, 4) << " deg\n";
  } else {
...
class BallisticApp final : public oe::ImGuiEventHandler {
 public:
  ...
  ~BallisticApp() override {
    if (solverControl_ != nullptr) {
      solverControl_->cancel();
    }
    if (worker_.joinable()) {
      worker_.join();
    }
 ...
 private:
  ...
  void publish(const ballistics::LiveState& s) {
    ballistics::LiveState adjusted = s;
    if (pauseRequested_.load() &&
        adjusted.status == ballistics::SolverStatus::Running) {
      adjusted.status = ballistics::SolverStatus::Paused;
      adjusted.message = "Solver paused. Press Continue to resume or Terminate to stop.";
    } else if (terminateRequested_.load() &&
               adjusted.status == ballistics::SolverStatus::Running) {
      adjusted.message = "Stopping solver...";
    }
    {
      std::lock_guard<std::mutex> lock(mutex_);
      state_ = adjusted;
  ...
  void publishLocalState(ballistics::SolverStatus status,
                         const std::string& message) {
    ballistics::LiveState localState;
    {
      std::lock_guard<std::mutex> lock(mutex_);
      localState = state_;
  ...
  void pauseSolve() {
    if (!busy_.load() || solverControl_ == nullptr || pauseRequested_.exchange(true)) {
      return;
    }
    solverControl_->requestPause();
    publishLocalState(ballistics::SolverStatus::Paused,
                      "Solver paused. Press Continue to resume or Terminate to stop.");
  }

  void resumeSolve() {
    if (!busy_.load() || solverControl_ == nullptr || !pauseRequested_.exchange(false)) {
      return;
    }
    solverControl_->resume();
    publishLocalState(ballistics::SolverStatus::Running, "Resuming solver...");
  }

  void terminateSolve() {
    if (!busy_.load() || solverControl_ == nullptr || terminateRequested_.exchange(true)) {
      return;
    }
    pauseRequested_.store(false);
    solverControl_->cancel();
    publishLocalState(ballistics::SolverStatus::Running, "Stopping solver...");
  }
  ...
  void startSolve() {
    ...
    if (run.search.alphaMinDeg > run.search.alphaMaxDeg) {
      std::swap(run.search.alphaMinDeg, run.search.alphaMaxDeg);
    }
    if (run.search.betaMinDeg > run.search.betaMaxDeg) {
      std::swap(run.search.betaMinDeg, run.search.betaMaxDeg);
    }
    if (run.search.controlHeightMinM > run.search.controlHeightMaxM) {
      std::swap(run.search.controlHeightMinM, run.search.controlHeightMaxM);
    }
    ...
    pauseRequested_.store(false);
    terminateRequested_.store(false);
    solverControl_ = std::make_shared<ballistics::SolverControl>();
    {
      std::lock_guard<std::mutex> lock(mutex_);
    ...
    const std::shared_ptr<ballistics::SolverControl> solverControl = solverControl_;
    worker_ = std::thread([this, run, solverControl]() {
      try {
        ballistics::LiveWriter writer([this](const ballistics::LiveState& s) { publish(s); });
        const auto result = ballistics::runSolver(run, &writer, solverControl.get());
        {
          std::lock_guard<std::mutex> lock(mutex_);
          solvedConfig_ = run;
      ...
      } catch (const ballistics::SolverInterrupted& ex) {
        ballistics::LiveState cancelled;
        cancelled.status = ballistics::SolverStatus::Cancelled;
        cancelled.message = ex.what();
        publish(cancelled);
  ...
  void drawNumericalNotes(const ballistics::LiveState& state, bool done) const {
    ...
    if (state.status == ballistics::SolverStatus::Cancelled) {
      if (!hasNotes) {
        ImGui::SeparatorText("Numerical Notes");
      }
      ImGui::TextColored(
          ImVec4(1.0f, 0.76f, 0.46f, 1.0f),
          "The current computation was terminated before completion.");
    }
  }
  ...
  void rebuildScene(const ballistics::LiveState& state, double animationProgress = 1.0) {
    ...
    if (state.attackBoundary.size() >= 3) {
      const auto outerLoop = scaledBoundaryLoop(state.attackBoundary, 12, 1.0, 0.05, 1'300.0);
      const auto ring75 = scaledBoundaryLoop(state.attackBoundary, 12, 0.75, 0.04, 1'150.0);
      const auto ring50 = scaledBoundaryLoop(state.attackBoundary, 12, 0.50, 0.03, 1'000.0);
      const auto ring25 = scaledBoundaryLoop(state.attackBoundary, 12, 0.25, 0.025, 900.0);
      const auto footprint = scaledBoundaryLoop(state.attackBoundary, 12, 1.0, 0.018, 750.0);
  ...
  void drawPanel(const ballistics::LiveState& state) {
    ...
    switch (state.status) {
      case ballistics::SolverStatus::Idle:
        statusText = "Idle";
        break;
      case ballistics::SolverStatus::Running:
        statusText = "Running";
        break;
      case ballistics::SolverStatus::Paused:
        statusText = "Paused";
        break;
      case ballistics::SolverStatus::Done:
        statusText = "Done";
        break;
      case ballistics::SolverStatus::Cancelled:
        statusText = "Cancelled";
        break;
      case ballistics::SolverStatus::Failed:
        statusText = "Failed";
        break;
    }
    ...
    InputDouble("Initial theta0 (deg)", &config_.vehicle.initialThetaDeg, 0.1, "%.3f");
    ImGui::SameLine();
    HelpMarker("theta0 is the initial velocity elevation angle relative to the local horizontal plane. Positive is upward.");
    InputDouble("Initial psi_v0 (deg)", &config_.vehicle.initialPsiDeg, 0.1, "%.3f");
    ImGui::SameLine();
    HelpMarker("psi_v0 is the ground-projected azimuth measured clockwise from north: 0 deg = north, 90 deg = east, 180 deg = south, 270 deg = west.");
    const std::string initialDirectionText =
        InitialDirectionText(config_.vehicle.initialThetaDeg,
                             config_.vehicle.initialPsiDeg);
    ImGui::TextDisabled("%s", initialDirectionText.c_str());
    ...
    if (ImGui::CollapsingHeader("More Runtime Parameters")) {
      ...
      InputDouble("Alpha min (deg)", &config_.search.alphaMinDeg, 0.1, "%.3f");
      InputDouble("Alpha max (deg)", &config_.search.alphaMaxDeg, 0.1, "%.3f");
      InputDouble("Beta min (deg)", &config_.search.betaMinDeg, 0.1, "%.3f");
      InputDouble("Beta max (deg)", &config_.search.betaMaxDeg, 0.1, "%.3f");
      InputDouble("h_c min (m)", &config_.search.controlHeightMinM, 100.0, "%.1f");
      InputDouble("h_c max (m)", &config_.search.controlHeightMaxM, 100.0, "%.1f");
    ...
    if (!hasEnabledMethods()) {
      ...
      if (pauseRequested_.load()) {
        if (ImGui::Button("Continue", ImVec2(halfWidth, 34.0f))) {
          resumeSolve();
        }
      } else {
        if (ImGui::Button("Pause", ImVec2(halfWidth, 34.0f))) {
          pauseSolve();
        }
      }
      ImGui::SameLine();
      if (ImGui::Button("Terminate", ImVec2(-1.0f, 34.0f))) {
        terminateSolve();
      }
      const char* runningText =
          pauseRequested_.load()
              ? "Solver is paused."
              : terminateRequested_.load() ? "Solver is stopping..." : "Solver is running...";
      ImGui::TextColored(ImVec4(0.50f, 0.90f, 1.0f, 1.0f), "%s", runningText);
    ...
    if (ImGui::BeginTable("metrics-table", 2,
      ...
      KeyValueRow("Current theta", FormatText("%.3f deg", currentTheta),
      ...
      KeyValueRow("Current range", FormatText("%.3f km", currentRange),
                  ImVec4(1.0f, 0.92f, 0.15f, 1.0f));
      KeyValueRow("Best theta", FormatText("%.3f deg", bestTheta),
    ...
    if (done) {
      ...
      if (ImGui::BeginTable("computed-data-table", 2,
        ...
        KeyValueRow("Initial direction",
                    InitialDirectionText(solvedConfig.vehicle.initialThetaDeg,
                                         solvedConfig.vehicle.initialPsiDeg));
        KeyValueRow("Control bounds",
                    FormatText("alpha [%.1f, %.1f] | beta [%.1f, %.1f] deg",
                               solvedConfig.search.alphaMinDeg,
                               solvedConfig.search.alphaMaxDeg,
                               solvedConfig.search.betaMinDeg,
                               solvedConfig.search.betaMaxDeg));
        KeyValueRow("Terminal state",
        ...
        if (solvedConfig.target.enabled) {
          KeyValueRow("Attack zone", "Skipped in target-point mode");
        } else {
    ...
    if (done && state.finalResult.targetSolution.enabled) {
      ...
      if (ImGui::BeginTable("target-table", 2,
        ...
        KeyValueRow("Closest miss distance",
                    FormatText("%.3f km", t.missDistance / 1000.0));
        if (std::isfinite(t.impactDistance)) {
          KeyValueRow("Impact miss distance",
                      FormatText("%.3f km", t.impactDistance / 1000.0));
        }
        KeyValueRow("Closest theta", FormatText("%.3f deg", t.thetaDeg));
        KeyValueRow("Closest heading", FormatText("%.3f deg", t.psiDeg));
        if (!t.reachable) {
  ...
  std::thread worker_;
  std::shared_ptr<ballistics::SolverControl> solverControl_;
